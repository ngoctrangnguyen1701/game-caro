...
const initialState = {
  ...
  tokenSwap: {
    ...
    abi: [{ "inputs": [{ "internalType": "address", "name": "_exToken", "type": "address" }, { "internalType": "address", "name": "_newToken", "type": "address" }], "stateMutability": "nonpayable", "type": "constructor" }, { "inputs": [], "name": "exToken", "outputs": [{ "internalType": "contract IERC20", "name": "", "type": "address" }], "stateMutability": "view", "type": "function" }, { "inputs": [], "name": "newToken", "outputs": [{ "internalType": "contract IERC20", "name": "", "type": "address" }], "stateMutability": "view", "type": "function" }, { "inputs": [], "name": "owner", "outputs": [{ "internalType": "address", "name": "", "type": "address" }], "stateMutability": "view", "type": "function" }, { "inputs": [{ "internalType": "uint256", "name": "amount", "type": "uint256" }], "name": "swap", "outputs": [], "stateMutability": "nonpayable", "type": "function" }]
  }
}


const ContractContextProvider = ({ children }) => {
  const [contractList, setContactList] = React.useState(initialState)
  const web3 = useSelector(state => state.web3.provider)

  const interactContact = async(contractName) => {
    if(web3) {
      const connectContract = await new web3.eth.Contract(contractList[contractName].abi, contractList[contractName].address)
      const newObj = {...contractList[contractName], contract: connectContract}
      setContactList({
        ...contractList,
        [contractName] : newObj
      })
  ...
  return (
    <ContractContext.Provider value={{contractList, interactContact}}>
      {children}
